...
def get_weather_and_garbage(user_id):
    # ユーザーの設定を取得（未登録ならデフォルトで北区、水土ルート）
    ...
    try:
        ...
        msg = f"【{area_name}の案内】\n"
        msg += f"（設定：燃やせる={b_days_str} / プラ={p_days_str}）\n\n"
...
def handle_message(event):
    ...
    # 1. 「初期設定」またはボットを初めて使うとき
    if user_message == "初期設定" or user_message == "設定":
        reply_text = "お住まいの「区」をタップして選択してください！"
        # 札幌の区をボタンとして並べる（画面の下からシュッと出ます）
        quick_reply_items = [
            QuickReplyItem(action=MessageAction(label=ward, text=f"区選択:{ward}")) for ward in SAPPORO_WARDS[:10]
        ]
        
    # 2. 区が選ばれたら、次は「燃やせるゴミの曜日」を聞く
    ...
    # 3. 燃やせるゴミの曜日が選ばれたら、次は「プラスチックの曜日」を聞く
    ...
    # 4. プラスチックが選ばれたら設定完了！
        ...
        p_str = user_message.split(":")[1][0] # 「月曜」の「月」だけ取得
        ...
        # プラの曜日から、自動的に「資源ゴミ（雑がみ等）」の曜日も予測してセット（札幌市はプラの翌日か前日が多いですが、ここでは簡易的にプラの曜日をベースにします）
        # 例としてプラが木曜(3)なら資源は金曜(4)のように1日ずらして仮セット
        USER_SETTINGS[user_id]["resource"] = [(plastic_day[0] + 1) % 7]
        
        reply_text = "🎉 すべての設定が完了しました！\n\n次からは、何か文字（「あ」など）を送るだけで、あなた専用の3日分の天気とゴミ出し情報をいつでも確認できます。"

    # 5. 通常時（何か文字が送られたら、登録された設定に基づいて3日分の情報を返す）
    else:
        if user_id in USER_SETTINGS:
            reply_text = get_weather_and_garbage(user_id)
        else:
            reply_text = "ご利用ありがとうございます！\nまずはあなたの地域を設定しましょう。\n\n下のボタンを押すか、「初期設定」とメッセージを送ってください。"
